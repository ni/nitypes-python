...
def is_dtype(dtype: npt.DTypeLike, supported_dtypes: tuple[npt.DTypeLike, ...]) -> bool:
    ...
    >>> import numpy as np; import pytest  # doctest: +SKIP
    >>> version_tuple = tuple(map(int, np.__version__.split('.')[:2]))  # doctest: +SKIP
    >>> if version_tuple < (2, 0): pytest.skip("requires numpy>=2.0")  # doctest: +SKIP
